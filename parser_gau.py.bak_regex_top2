...
def read_Top(all_lines, ric_list):
    ...
    Patched version:
    - skips empty topology fields
    - avoids IndexError from tmp_list[i][0]
    - parses R(...), A(...), D(...) entries more robustly
    """
    tmp_list = []
    ric_tot = ric_list[0]

    found_name_block = False
    for s in range(len(all_lines)):
        if 'Name' in all_lines[s]:
            found_name_block = True
            start = s
            for e in range(start + 2, min(start + ric_tot + 2, len(all_lines))):
                item = all_lines[e][8:25].strip()
                if item:
                    tmp_list.append(item)
            break

    if not found_name_block:
        raise RuntimeError("Could not find Gaussian RIC topology block containing 'Name'.")
    ...
    for item in tmp_list:
        ...
        if '(' in item and ')' in item:
            inside = item[item.find('(') + 1:item.rfind(')')]
        else:
            inside = item[1:]
        ...
        if tag == 'R':
            bond_list.append(parts)
        elif tag == 'A':
            angle_list.append(parts)
        elif tag == 'D':
            dihe_list.append(parts)

    bond_list = [list(map(int, x)) for x in bond_list if x]
    angle_list = [list(map(int, x)) for x in angle_list if x]
    dihe_list = [list(map(int, x)) for x in dihe_list if x]

    print("Parsed topology from Gaussian RIC block:")
