...
"""
    ...
    # Robust charge fallback:
    # The coordinate loop uses zip(ele_list, type_list, coord, charges).
    # If charges is empty, zip writes zero atoms. Do not allow that.
    if charges is None or len(charges) != len(ele_list):
        print(
            f"WARNING print_GauInp: charge length mismatch: "
            f"charges={0 if charges is None else len(charges)}, atoms={len(ele_list)}"
        )
        print("WARNING print_GauInp: using temporary +0.000000 charges for hessfit4gau.gjf.")
        charges = ["+0.000000"] * len(ele_list)

    with open(filename, 'w') as file_out:
        file_out.write(header_gjf.format(f_chg = formal_chg, mult=multi))

        n_written = 0
        for element, atom_type, coordinates, charge in zip(ele_list, type_list, coord, charges):
            formatted_coords = '  '.join(f'{x:.6f}' for x in coordinates)
            file_out.write(f'{element}-{atom_type}-{charge}  {formatted_coords}\n')
            n_written += 1

        if n_written == 0:
            raise RuntimeError("print_GauInp wrote zero atoms to hessfit4gau.gjf")
