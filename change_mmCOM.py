#!/usr/bin/env python3

import parser_gau as pgau
import force_constant_mod as fc
import numpy as np
import average_across_types as aat
import readin_opts as rdin
import os
import geom2atype as g2a

def print_GauHarm(*args):
    ele_ls, tp_ls, coord, bond_tp_ls, k_bond_ls, bond_ln_ls, angle_tp_ls, k_angle_ls, \
    angle_ln_ls, torsion_type_list, hybrid_list, chg, formal_chg, multi, *extra = args
    mem = extra[0] if len(extra) >= 1 else "1GB"
    nprocs = extra[1] if len(extra) >= 2 else 1

    header_gjf = """%mem={mem}
%nprocshared={nprocs}
%chk=GauHarm.chk
#p Amber=(SoftOnly,Print) nosymm  Freq=intmodes
 
Title

{f_chg} {mult}
"""

    master_func = """
! Master function
NonBon 3 1 0 0 0.000 0.000 0.500 0.000 0.000 -1.2
"""
    fname = 'GauHarm.gjf'
    with open(fname, 'w') as fout:
        fout.write(header_gjf.format(mem=mem, nprocs=nprocs, f_chg=formal_chg, mult=multi))
        for m, p, l in zip(ele_ls, tp_ls, coord):
            s2 = '  '.join(f'{x:.6f}' for x in l)
            fout.write(f'{m}-{p}-0.00  {s2}\n')

        fout.write(master_func)
        fout.write('! SMARTFIELD FF\n')
        fout.write('!Bonds\n')
        for bond_tp, k_bond, bond_ln in zip(bond_tp_ls, k_bond_ls, bond_ln_ls):
            fout.write(f'HrmStr1 {bond_tp}  1.0 {bond_ln:.3f}\n')

        fout.write('!Angles\n')
        for angle_tp, k_angle, angle_ln in zip(angle_tp_ls, k_angle_ls, angle_ln_ls):
            fout.write(f'HrmBnd1 {angle_tp}  1.0 {angle_ln:.3f}\n')
            
        fout.write('!Torsions\n')
        for i, (torsion_type, hybrid) in enumerate(zip(torsion_type_list, hybrid_list)):
            if hybrid == 4.0 or hybrid == 2.0:
               formatted_phase = '0 180 0 0 '
               fout.write(f'AmbTrs {torsion_type} {formatted_phase} 0. {1.0} 0. 0. {float(hybrid)}\n')
            else:
               formatted_phase = '0 0 0 0 '
               fout.write(f'AmbTrs {torsion_type} {formatted_phase} 0. 0. {1.0} 0. {float(hybrid)}\n')
            
            
        fout.write('\n')
        
        
        
        
        
def print_GauNonBon(*args):
    elements, types, coordinates, bond_types, bond_lengths, angle_types, \
    angle_lengths, torsion_type_list, hybrid_list,  charges, vdw, formal_chg, multi, *extra = args
    mem = extra[0] if len(extra) >= 1 else "1GB"
    nprocs = extra[1] if len(extra) >= 2 else 1
    header_gjf = """%mem={mem}
%nprocshared={nprocs}
%chk=GauNonBon.chk
#p Amber=(SoftOnly,Print) nosymm Freq=intmodes
 
Title

{f_chg} {mult}
"""

    master_func = """
! Master function
NonBon 3 1 0 0 0.000 0.000 0.500 0.000 0.000 -1.2
"""
    fname = 'GauNonBon.gjf'

    with open(fname, 'w') as fout:
        fout.write(header_gjf.format(mem=mem, nprocs=nprocs, f_chg=formal_chg, mult=multi))

        if not (len(elements) == len(types) == len(coordinates) == len(charges)):
            raise RuntimeError(
                "GauNonBon coordinate length mismatch: "
                f"elements={len(elements)}, types={len(types)}, "
                f"coordinates={len(coordinates)}, charges={len(charges)}"
            )

        n_written = 0
        for element, type_, coords, charge in zip(elements, types, coordinates, charges):
            coord_str = '  '.join(f'{x:.6f}' for x in coords)
            fout.write(f'{element}-{type_}-{charge}  {coord_str}\n')
            n_written += 1

        if n_written == 0:
            raise RuntimeError("print_GauNonBon wrote zero atoms.")

        fout.write(master_func)
        fout.write('! SMARTFIELD FF\n')
        fout.write('!Bonds\n')
        for bond_type, bond_length in zip(bond_types, bond_lengths):
            fout.write(f'HrmStr1 {bond_type}  0.0 {bond_length:.3f}\n')

        fout.write('!Angles\n')
        for angle_type, angle_length in zip(angle_types, angle_lengths):
            fout.write(f'HrmBnd1 {angle_type}  0.0 {angle_length:.3f}\n')
            
        fout.write('!Torsions\n')
        for i, (torsion_type, hybrid) in enumerate(zip(torsion_type_list, hybrid_list)):
            if hybrid == 4.0 or hybrid == 2.0:
               formatted_phase = '0 180 0 0 '
               fout.write(f'AmbTrs {torsion_type} {formatted_phase} 0. 0. 0. 0. {float(hybrid)}\n')
            else:
               formatted_phase = '0 0 0 0 '
               fout.write(f'AmbTrs {torsion_type} {formatted_phase} 0. 0. 0. 0. {float(hybrid)}\n')
            
        fout.write('!VDW\n')
        for vdw_params in vdw:
            s = ' '.join(map(str, vdw_params))
            fout.write(f'{s}\n')

        fout.write('\n')

def setup_gaussian_path(opts):
    """Set up and validate Gaussian installation path."""
    g09root = os.environ.get('g09root')
    g16root = os.environ.get('g16root')
    
    # Determine which Gaussian version to use
    if opts.version == 'g16':
        groot = g16root if g16root else None
    else:
        groot = g09root if g09root else None
    
    # Validate the selected Gaussian path
    if groot:
        GPATH = os.path.join(groot, opts.version)
        print(f"GPATH == {GPATH}")

        if not os.path.exists(GPATH):
            print(f"Warning: {GPATH} does not exist. Falling back to user-defined path.")
            groot = opts.path
            GPATH = os.path.join(groot, opts.version)
    else:
        groot = opts.path
        GPATH = os.path.join(groot, opts.version)
    
    if not os.path.exists(GPATH):
        raise FileNotFoundError(f"Error: No valid Gaussian installation found at {GPATH}.")
    
    return GPATH

def main():
    parser = rdin.commandline_parser3()
    parser.add_argument('--version', choices=['g09', 'g16'], default='g09', help='Select Gaussian version (g09 or g16)')
    parser.add_argument('--at', choices=["gaff", "amber", "scratch"], default="gaff", help='Select force field for VDW parameters (gaff or amber); default = gaff')
    opts = parser.parse_args()
    json_opts = rdin.read_optfile_3(opts.optfile)

    GPATH = setup_gaussian_path(opts)

    # opts = parser.parse_args()
    f_qm_log = json_opts['files']['log_qm_file']
    f_qm_fchk = json_opts['files']['fchk_qm_file']
    # f_atype = json_opts['files']['atype_file']
    formal_chg = json_opts['charge']
    multi = json_opts['multiplicity']
    mem = json_opts.get('mem', '1GB')
    nprocs = json_opts.get('nprocs', 1)
    print(f"Gaussian helper inputs will use %mem={mem}, %nprocshared={nprocs}")
    text_qm_log = pgau.store_any_file(f_qm_log)
    text_qm_fchk = pgau.store_any_file(f_qm_fchk)
    # text_atype = pgau.store_any_file(f_atype)
    
    N_atoms, qm_XYZ = pgau.read_XYZ(text_qm_fchk)


    fchk = pgau.read_fchk(f_qm_fchk)
    N_atoms = fchk["natoms"]
    qm_XYZ = fchk["coords"]
    ele_list = fchk["elements"]
    # # H_cart = fchk["H_cart"]
    # # masses = fchk["masses"]
    if opts.at == "amber":
        atype_list = g2a.assign_amber_atom_types(ele_list, qm_XYZ) # overwriting existing atype from json
    elif opts.at == "gaff":
        atype_list = g2a.assign_gaff_atom_types(ele_list, qm_XYZ)
    elif opts.at == "scratch":
        atype_list = []                                      # build atom types if none provided
        for i, var in enumerate(ele_list):
            atype_list.append(''.join(f'{var}{i}'))
        atype_list = pgau.flat_list(atype_list) 

    print("The following atom types have been assigned:")
    print(' '.join(f"{el}-{at} ; " for el, at in zip(ele_list, atype_list)))


    # ele_list, type_list = pgau.read_NamesTypes(text_qm_log, N_atoms)
    ric_list, force_1D = pgau.read_RicDim_Grad(text_qm_fchk)
    No_ric = ric_list[0]
    No_bonds = ric_list[1]
    No_angles = ric_list[2]
    No_dihes = ric_list[3]

    # # Reading in Topology in RIC from log file
    chg = pgau.read_CM5(text_qm_log, N_atoms)

    # Robust fallback: if Gaussian09/RevC.01 does not provide CM5/Mulliken
    # in the expected parser format, avoid writing an empty GauNonBon.gjf.
    if len(chg) != N_atoms:
        print(f"WARNING: read_CM5 returned {len(chg)} charges for {N_atoms} atoms.")
        print("WARNING: Using temporary zero charges for HessFit Gaussian build files.")
        print("WARNING: Replace final MD charges later with proper RESP/MSK/ATB/CM5 charges.")
        chg = ["+0.000000"] * N_atoms

    bond_list, angle_list, tors_list = pgau.read_Top(text_qm_log, ric_list)
    k_bonds = np.ones(No_bonds)                          
    k_angles = np.ones(No_angles)
    k_tors = np.ones(No_dihes)
    # print(No_dihes)
    
    mdin = json_opts['opt']
    mode = json_opts['mode']
    bond_type_list, bond_arr, k_bond_arr = fc.set_bonds(qm_XYZ, None, atype_list, \
                      bond_list, k_bonds, mdin, mode)
    angle_type_list, angle_arr, k_angle_arr = fc.set_angles(qm_XYZ, None, atype_list, \
                      angle_list, k_angles, mdin, mode)
    tors_type_list, v1, _, _, _, _, periodic_list = fc.set_torsion(qm_XYZ, atype_list, tors_list, k_tors, force_1D, mode)
    
    # Take out Mirror atom types of bonds & angles
    bond_reduced, _ = aat.make_list_unique(bond_type_list, k_bond_arr)
    angle_reduced, _ = aat.make_list_unique(angle_type_list, k_angle_arr)
    tors_reduced, hybrid_unique = aat.make_list_unique(tors_type_list, periodic_list)
    
    # Print all into Gaussian Input
    print_GauHarm(ele_list, atype_list, qm_XYZ, \
                 bond_reduced, k_bond_arr, bond_arr, \
                 angle_reduced, k_angle_arr, angle_arr, \
                 tors_reduced, hybrid_unique, \
                 chg, formal_chg, multi, mem, nprocs)
    
    # path = os.environ.get("g09root") + "/g09"
    VDW_list = pgau.read_AmberParm(GPATH, atype_list)
    
    print(f"DEBUG GauNonBon lengths: elements={len(ele_list)}, types={len(atype_list)}, coords={len(qm_XYZ)}, charges={len(chg)}, vdw={len(VDW_list)}")

    print_GauNonBon(ele_list, atype_list, qm_XYZ, \
                 bond_reduced, bond_arr, \
                 angle_reduced, angle_arr, \
                 tors_reduced, hybrid_unique, \
                 chg, VDW_list, formal_chg, multi)




if __name__ == '__main__':
    main()